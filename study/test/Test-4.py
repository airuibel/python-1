...
def get_douban_comments(url):
    ...
    user_name = 'cysuncn@126.com'  # 这里替换成你的豆瓣用户名
    password = 'ctl7220fe'  # 这里替换成你的密码
    ...
    while True:
        try:
            ...
            print e
            break
    with codecs.open('D:\Devlop\Python\python\pjl_comment.txt','a',encoding='utf-8') as f:
...
# 绘制词云
def draw_wordcloud():
   with codecs.open('D:\Devlop\Python\python\pjl_comment.txt',encoding='utf-8') as f:
       comment_text = f.read()
   cut_text = " ".join(jieba.cut(comment_text)) # 将jieba分词得到的关键词用空格连接成为字符串
   d = path.dirname(__file__) # 当前文件文件夹所在目录
   color_mask = imread("D:\Devlop\Python\python\heart.png") # 读取背景图片
...
if __name__ == '__main__':
    ...
    file_name = 'D:\Devlop\Python\python\pjl_comment.txt'
