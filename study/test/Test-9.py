...
class Game(object):
    ...
    def loadtext(self, sorce):
        self.my_font = pygame.font.SysFont(None, 22)
        textstr = 'sum:' + str(sorce)
...
def convolutional_neural_network(input_image):
    weights = {'w_conv1': tf.Variable(tf.zeros([8, 8, 4, 32])), 'w_conv2': tf.Variable(tf.zeros([4, 4, 32, 64])),
               'w_conv3': tf.Variable(tf.zeros([3, 3, 64, 64])), 'w_fc4': tf.Variable(tf.zeros([3456, 784])),
               'w_out': tf.Variable(tf.zeros([784, output]))}

    biases = {'b_conv1': tf.Variable(tf.zeros([32])), 'b_conv2': tf.Variable(tf.zeros([64])),
              'b_conv3': tf.Variable(tf.zeros([64])), 'b_fc4': tf.Variable(tf.zeros([784])),
...
def train_neural_network(input_image):
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            n = n + 1
            if n % 10000 == 0:
                saver.save(sess, 'D:\model\game.cpk', global_step=n)  # 保存模型
