...
st = datetime.now()
conf = SparkConf().setAppName('PROC_A_SUBJECT_D004037').setMaster(sys.argv[2])
...
sql = """
 SELECT 
			...
			,A.FR_ID			as FR_ID
	FROM ACRM_A_TARGET_D004006 A,
		 ACRM_A_TARGET_D004007 B,
		 ACRM_A_TARGET_D004008 C
...
ACRM_A_TARGET_D004037 = sqlContext.sql(sql)
ACRM_A_TARGET_D004037.registerTempTable("ACRM_A_TARGET_D004037")
dfn="ACRM_A_TARGET_D004037/"+V_DT+".parquet"
ACRM_A_TARGET_D004037.cache()
nrows = ACRM_A_TARGET_D004037.count()
ACRM_A_TARGET_D004037.write.save(path=hdfs + '/' + dfn, mode='overwrite')
ACRM_A_TARGET_D004037.unpersist()
ACRM_A_TARGET_D004006.unpersist()
ACRM_A_TARGET_D004007.unpersist()
ACRM_A_TARGET_D004008.unpersist()
ret = os.system("hdfs dfs -rm -r /"+dbname+"/ACRM_A_TARGET_D004037/"+V_DT_LD+".parquet")
et = datetime.now()
print("Step %d start[%s] end[%s] use %d seconds, insert ACRM_A_TARGET_D004037 lines %d") % (V_STEP, st.strftime("%H:%M:%S"), et.strftime("%H:%M:%S"), (et-st).seconds, nrows)
