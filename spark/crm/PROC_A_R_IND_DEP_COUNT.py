...
st = datetime.now()
conf = SparkConf().setAppName('PROC_A_R_IND_DEP_COUNT').setMaster(sys.argv[2])
...
#保留当天及月末数据
if V_DT_LD != V_DT_LMD :
	ret = os.system("hdfs dfs -rm -r /"+dbname+"/ACRM_A_IND_DEP_COUNT/"+V_DT_LD+".parquet")

#删除当天的
ret = os.system("hdfs dfs -rm -r /"+dbname+"/ACRM_A_IND_DEP_COUNT/"+V_DT+".parquet")
...
ACRM_A_IND_DEP_COUNT = sqlContext.sql(sql)
ACRM_A_IND_DEP_COUNT.registerTempTable("ACRM_A_IND_DEP_COUNT")
dfn="ACRM_A_IND_DEP_COUNT/"+V_DT+".parquet"
ACRM_A_IND_DEP_COUNT.cache()
nrows = ACRM_A_IND_DEP_COUNT.count()
ACRM_A_IND_DEP_COUNT.write.save(path=hdfs + '/' + dfn, mode='append')
ACRM_A_IND_DEP_COUNT.unpersist()
ret = os.system("hdfs dfs -rm -r /"+dbname+"/ACRM_A_IND_DEP_COUNT/"+V_DT_LD+".parquet")
et = datetime.now()
print("Step %d start[%s] end[%s] use %d seconds, insert ACRM_A_IND_DEP_COUNT lines %d") % (V_STEP, st.strftime("%H:%M:%S"), et.strftime("%H:%M:%S"), (et-st).seconds, nrows)
