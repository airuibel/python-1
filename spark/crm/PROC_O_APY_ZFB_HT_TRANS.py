...
st = datetime.now()
conf = SparkConf().setAppName('PROC_O_APY_ZFB_HT_TRANS').setMaster(sys.argv[2])
...
O_TX_ZFB_HT_TRANS = sqlContext.read.parquet(hdfs+'/O_TX_ZFB_HT_TRANS/*')
O_TX_ZFB_HT_TRANS.registerTempTable("O_TX_ZFB_HT_TRANS")

#任务[21] 001-01::
V_STEP = V_STEP + 1

sql = """
 SELECT TRANS_SEQNO             AS TRANS_SEQNO 
       ...
       ,V_DT                    AS ODS_ST_DATE 
       ,'APY'                   AS ODS_SYS_ID 
   FROM O_TX_ZFB_HT_TRANS A                                    --交易明细表（历史）
"""

sql = re.sub(r"\bV_DT\b", "'"+V_DT10+"'", sql)
F_TX_ZFB_HT_TRANS = sqlContext.sql(sql)
F_TX_ZFB_HT_TRANS.registerTempTable("F_TX_ZFB_HT_TRANS")
dfn="F_TX_ZFB_HT_TRANS/"+V_DT+".parquet"
F_TX_ZFB_HT_TRANS.cache()
nrows = F_TX_ZFB_HT_TRANS.count()
F_TX_ZFB_HT_TRANS.write.save(path=hdfs + '/' + dfn, mode='overwrite')
F_TX_ZFB_HT_TRANS.unpersist()
ret = os.system("hdfs dfs -rm -r /"+dbname+"/F_TX_ZFB_HT_TRANS/"+V_DT_LD+".parquet")
et = datetime.now()
print("Step %d start[%s] end[%s] use %d seconds, insert F_TX_ZFB_HT_TRANS lines %d") % (V_STEP, st.strftime("%H:%M:%S"), et.strftime("%H:%M:%S"), (et-st).seconds, nrows)
