...
st = datetime.now()
conf = SparkConf().setAppName('PROC_F_WP_REMIND_OVERDUE_DEP').setMaster(sys.argv[2])
...
sql = """
 SELECT monotonically_increasing_id()     AS ID 
  ...
  WHERE E.REMIND_TYPE           = 'A000111' 
    AND DATE(B.LTDT)                      = V_DT """

sql = re.sub(r"\bV_DT\b", "'"+V_DT10+"'", sql)
OCRM_F_WP_REMIND_OVERDUE_DEP = sqlContext.sql(sql)
OCRM_F_WP_REMIND_OVERDUE_DEP.registerTempTable("OCRM_F_WP_REMIND_OVERDUE_DEP")
dfn="OCRM_F_WP_REMIND_OVERDUE_DEP/"+V_DT+".parquet"
OCRM_F_WP_REMIND_OVERDUE_DEP.cache()
nrows = OCRM_F_WP_REMIND_OVERDUE_DEP.count()
#删除当天，支持重跑
ret = os.system("hdfs dfs -rm -r /"+dbname+"/OCRM_F_WP_REMIND_OVERDUE_DEP/"+V_DT+".parquet")
OCRM_F_WP_REMIND_OVERDUE_DEP.write.save(path=hdfs + '/' + dfn, mode='append')
OCRM_F_WP_REMIND_OVERDUE_DEP.unpersist()
et = datetime.now()
print("Step %d start[%s] end[%s] use %d seconds, insert OCRM_F_WP_REMIND_OVERDUE_DEP lines %d") % (V_STEP, st.strftime("%H:%M:%S"), et.strftime("%H:%M:%S"), (et-st).seconds, nrows)
