...
st = datetime.now()
conf = SparkConf().setAppName('PROC_F_WP_REMIND_OVER_GUADEP').setMaster(sys.argv[2])
...
sql = """
 SELECT monotonically_increasing_id()     AS ID 
  ...
  WHERE E.REMIND_TYPE           = 'A000112' """

sql = re.sub(r"\bV_DT\b", "'"+V_DT10+"'", sql)
OCRM_F_WP_REMIND_OVER_GUADEP = sqlContext.sql(sql)
OCRM_F_WP_REMIND_OVER_GUADEP.registerTempTable("OCRM_F_WP_REMIND_OVER_GUADEP")
dfn="OCRM_F_WP_REMIND_OVER_GUADEP/"+V_DT+".parquet"
OCRM_F_WP_REMIND_OVER_GUADEP.cache()
nrows = OCRM_F_WP_REMIND_OVER_GUADEP.count()
OCRM_F_WP_REMIND_OVER_GUADEP.write.save(path=hdfs + '/' + dfn, mode='overwrite')
OCRM_F_WP_REMIND_OVER_GUADEP.unpersist()
#ret = os.system("hdfs dfs -rm -r /"+dbname+"/OCRM_F_WP_REMIND_OVER_GUADEP/"+V_DT_LD+".parquet")
et = datetime.now()
print("Step %d start[%s] end[%s] use %d seconds, insert OCRM_F_WP_REMIND_OVER_GUADEP lines %d") % (V_STEP, st.strftime("%H:%M:%S"), et.strftime("%H:%M:%S"), (et-st).seconds, nrows)
