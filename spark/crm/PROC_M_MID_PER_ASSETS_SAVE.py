...
st = datetime.now()
conf = SparkConf().setAppName('PROC_M_MID_PER_ASSETS_SAVE').setMaster(sys.argv[2])
...
sql = """
 SELECT A.CUST_ID               AS CUST_ID 
   ...
   LEFT JOIN ACRM_F_RE_SAVESUMAVGINFO B                        --ACRM_F_RE_SAVESUMAVGINFO
...
TMP_PER_ASSETS_SAVE = sqlContext.sql(sql)
TMP_PER_ASSETS_SAVE.registerTempTable("TMP_PER_ASSETS_SAVE")
dfn="TMP_PER_ASSETS_SAVE/"+V_DT+".parquet"
TMP_PER_ASSETS_SAVE.cache()
nrows = TMP_PER_ASSETS_SAVE.count()
TMP_PER_ASSETS_SAVE.write.save(path=hdfs + '/' + dfn, mode='overwrite')
TMP_PER_ASSETS_SAVE.unpersist()

#全量表保存后需要删除前一天数据
ret = os.system("hdfs dfs -rm -r /"+dbname+"/TMP_PER_ASSETS_SAVE/"+V_DT_LD+".parquet")
et = datetime.now()
print("Step %d start[%s] end[%s] use %d seconds, insert TMP_PER_ASSETS_SAVE lines %d") % (V_STEP, st.strftime("%H:%M:%S"), et.strftime("%H:%M:%S"), (et-st).seconds, nrows)
